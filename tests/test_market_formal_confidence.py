...
def test_scored_cache_version_is_decoupled_from_evaluation_output(tmp_path) -> None:
    ...
    assert MARKET_EVALUATION_VERSION == 35
    assert MARKET_MAX_SNAPSHOT_AGE_SECONDS == 65.0
    assert contract["version"] == SCORED_CACHE_VERSION == 15
    ...
    assert contract["decision_time_boundary_contract_version"] == 1
    assert contract["required_decision_lead_seconds"] == 300
