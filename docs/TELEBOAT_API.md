...
- 冪等性キーのSHA-256予約はhash-chainジャーナルへファイルロック内で保存され、API再起動後や複数ワーカー間でも重複要求を拒否します。
- 最終ボタン押下後に結果確認不能となった要求は再試行禁止です。同じ冪等性キーは保持されます。最終ボタン前の失敗がジャーナルに記録できた場合だけ、同じキーの再試行を許可します。
...
投票開始前は、読み取り専用プローブで公式の投票可能残高とログアウト完了を確認します。
ログイン直後がお知らせ画面の場合は公式の「トップへ」だけを経由し、投票操作は行いません。

~~~bash
teleboat-balance-probe --secrets .secrets/teleboat-login.json
~~~

投票本体も同じ確認を必須実行します。残高を取得できない場合、または要求の購入総額に
満たない場合は、ベットリストを操作する前に失敗として停止します。
...
6. 公式の投票可能残高を取得し、要求の購入総額以上であることを確認。
7. 場、レース、勝式、方式のradioを設定後に値を再確認。
8. 各組番・数量を入力後にinput_valueを再確認。
9. BOX/フォーメーションは公式表示とローカルの展開点数を照合。
10. ベットリストで場、レース、勝式、全組番、点数、購入金額、「投票未完了」を照合。
11. 公式hidden合計金額とローカル合計を照合。
12. live要求だけ、最終購入金額を入力・再確認して最終ボタンを押下。
13. 受付完了表示を確認し、ログアウトを試行。
