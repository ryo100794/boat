...
from .modeling_no_odds_v8 import FEATURE_SET, make_pipeline, positive_probs


def configure_v8_bankroll() -> None:
    base.load_training_examples = load_training_examples
    base._make_pipeline = make_pipeline
    base._positive_probs = positive_probs


def bankroll_backtest_v8(
    conn,
    *,
    output_path: Path,
    ...
    result["model"] = "win_model_no_odds_v8"
    result["policy"]["model"] = "win_model_no_odds_v8"
    result["policy"]["feature_set"] = FEATURE_SET
    result["policy"]["odds_note"] = (
        "historical pre-race odds are not available in the archive; "
        "EV uses train-fold average trifecta payouts, while live guidance uses latest collected odds."
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Backtest daily bankroll buying with no-odds v8 model.")
    parser.add_argument("--db", default="data/boatrace.sqlite")
    parser.add_argument("--output", default="data/models/bankroll_backtest_no_odds_v8_10000.json")
