...
from .features_pastlog_v1 import load_training_examples, prediction_features
from .modeling_no_odds_v6 import SparseIndex32


FEATURE_SET = "pastlog_v1_card_rolling_history_logreg_C0.15"
...
def train_model(conn, *, model_path: Path, min_examples: int = 100) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "vectorizer": "sparse",
        "scaler": "MaxAbsScaler",
        "classifier": "LogisticRegression(liblinear, C=0.15, class_weight=None)",
        "feature_set": FEATURE_SET,
        "role": "primary_until_realtime_history_is_sufficient",
        "design_note": "uses card fields and leakage-aware rolling history; excludes realtime beforeinfo/weather/odds.",
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train/backtest the past-log primary model.")
    sub = parser.add_subparsers(dest="command", required=True)
    train = sub.add_parser("train")
    add_common(train)
    train.add_argument("--model", default="data/models/win_model_pastlog_v1.joblib")
    train.add_argument("--min-examples", type=int, default=100)
    train.set_defaults(func=_cmd_train)
    backtest = sub.add_parser("backtest")
    add_common(backtest)
    backtest.add_argument("--output", default="data/models/backtest_pastlog_v1.json")
