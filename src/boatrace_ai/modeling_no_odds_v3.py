...
from .dense_tasks import make_pipeline, positive_probs
from .features import latest_trifecta_odds
from .features_no_odds_v3 import load_training_examples, prediction_features
...
def train_model(
    ...
    *,
    ...
    metadata = {
        ...
        "vectorizer": "dense",
        "feature_set": "no_odds_v3_relative_racer_motor_boat_weather",
    }
...
def predict_race(
    ...
    *,
    ...
    lane_probs_raw = positive_probs(pipeline, X)
    lane_probs = _normalize_lane_probs({lane: lane_probs_raw[lane - 1] for lane in range(1, 7)})
    latest_odds = latest_trifecta_odds(conn, race_id_value)
    rows = trifecta_predictions(lane_probs, latest_odds=latest_odds)[:top_n]
    if store:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train/backtest no-odds v3 model.")
    sub = parser.add_subparsers(dest="command", required=True)
    train = sub.add_parser("train")
    train.add_argument("--db", default="data/boatrace.sqlite")
    train.add_argument("--model", default="data/models/win_model_no_odds_v3.joblib")
    train.add_argument("--min-examples", type=int, default=100)
    train.set_defaults(func=_cmd_train)
    backtest = sub.add_parser("backtest")
    backtest.add_argument("--db", default="data/boatrace.sqlite")
    backtest.add_argument("--output", default="data/models/backtest_no_odds_v3.json")
