...
FEATURE_SET = "no_odds_v7_relative_weather_sparse32_scaled_logreg_l2_unweighted"
...
def train_model(conn, *, model_path: Path, min_examples: int = 100) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "vectorizer": "sparse",
        "scaler": "MaxAbsScaler",
        "classifier": "LogisticRegression(liblinear, l2, C=0.5, class_weight=None)",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train/backtest no-odds v7 model.")
    sub = parser.add_subparsers(dest="command", required=True)
    train = sub.add_parser("train")
    add_common(train)
    train.add_argument("--model", default="data/models/win_model_no_odds_v7.joblib")
    train.add_argument("--min-examples", type=int, default=100)
    train.set_defaults(func=_cmd_train)
    backtest = sub.add_parser("backtest")
    add_common(backtest)
    backtest.add_argument("--output", default="data/models/backtest_no_odds_v7.json")
