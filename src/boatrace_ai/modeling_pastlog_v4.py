...
from .features_pastlog_v4 import load_training_examples, prediction_features
from .modeling_no_odds_v6 import SparseIndex32


FEATURE_SET = "pastlog_v4_series_form_prevday_rolling_history_logreg_C0.12"
...
def train_model(conn, *, model_path: Path, min_examples: int = 100) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "design_note": (
            "official B/K historical files plus raw_json series form; prunes low-coverage card "
            "fields and excludes realtime odds/weather."
        ),
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train/backtest past-log v4 model with official series-form features.")
    sub = parser.add_subparsers(dest="command", required=True)
    train = sub.add_parser("train")
    add_common(train)
    train.add_argument("--model", default="data/models/win_model_pastlog_v4.joblib")
    train.add_argument("--min-examples", type=int, default=100)
    train.set_defaults(func=_cmd_train)
    backtest = sub.add_parser("backtest")
    add_common(backtest)
    backtest.add_argument("--output", default="data/models/backtest_pastlog_v4.json")
