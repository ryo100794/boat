...
def parse_official_archive(conn, *, path: Path, kind: str, race_date: date) -> dict[str, int]:
    ...
    for filename, payload in extract_lzh(path):
        ...
        if kind == "program":
            parsed = parse_program_text(conn, text=text, race_date=race_date)
        elif kind == "result":
            parsed = parse_result_text(conn, text=text, race_date=race_date)
        else:
            parsed = {"races": 0, "entries": 0, "results": 0, "payouts": 0}
...
def parse_program_text(conn, *, text: str, race_date: date) -> dict[str, int]:
    ...
    for raw_line in text.splitlines():
        ...
        if race_match:
            ...
            current_race_id = race_id(race_date.isoformat(), current_jcd, current_rno)
...
def parse_program_entry(line: str) -> dict[str, Any] | None:
    ...
    head_match = re.match(r"(?P<name>.*?)(?P<age>\d{2})(?P<branch>.{2})(?P<weight>\d{2})$", head)
    if not head_match:
        return None
    return {
