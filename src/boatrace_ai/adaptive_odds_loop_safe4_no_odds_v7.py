...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Adaptive odds/results loop for no-odds v7.")
        ...
        with connection(args.db) as conn:
            for row in scheduled_races(conn, target_date):
                ...
                if args.collect_results:
                    ...
                    result_due = result_interval(seconds_to_deadline)
                    result_age = (
...
def result_interval(seconds_to_deadline: float) -> float | None:
    if seconds_to_deadline >= 0:
        return None
    elapsed = -seconds_to_deadline
    if elapsed <= 90 * 60:
        return 60.0
    if elapsed <= 6 * 60 * 60:
        return 300.0
