...
from .features_realtime_hybrid_v1 import load_training_examples
from .modeling_realtime_hybrid_v1 import FEATURE_SET, make_pipeline, positive_probs
...
def bankroll_backtest_hybrid(
    ...
    *,
    ...
    result["model"] = "win_model_realtime_hybrid_v1"
    result["policy"]["model"] = "win_model_realtime_hybrid_v1"
    result["policy"]["feature_set"] = FEATURE_SET
    result["policy"]["role"] = "shadow_realtime_hybrid"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Backtest daily bankroll buying with realtime hybrid shadow model.")
    parser.add_argument("--db", default="data/boatrace.sqlite")
    parser.add_argument("--output", default="data/models/bankroll_backtest_realtime_hybrid_v1_10000.json")
    ...
    with connection(args.db) as conn:
        result = bankroll_backtest_hybrid(
            conn,
