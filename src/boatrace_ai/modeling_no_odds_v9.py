...
from .features_no_odds_v9 import load_training_examples, prediction_features
from .modeling_no_odds_v6 import SparseIndex32


FEATURE_SET = "no_odds_v9_rolling_racer_motor_boat_context_logreg_C0.12"
...
def train_model(conn, *, model_path: Path, min_examples: int = 100) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "design_note": (
            "raw motor_no and boat_no are removed; racer, lane, venue, rno, motor, "
            "and boat history are represented as leakage-aware rolling rates."
        ),
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train/backtest no-odds v9 rolling-history tuning model.")
    sub = parser.add_subparsers(dest="command", required=True)
    train = sub.add_parser("train")
    add_common(train)
    train.add_argument("--model", default="data/models/win_model_no_odds_v9.joblib")
    train.add_argument("--min-examples", type=int, default=100)
    train.set_defaults(func=_cmd_train)
    backtest = sub.add_parser("backtest")
    add_common(backtest)
    backtest.add_argument("--output", default="data/models/backtest_no_odds_v9.json")
