...
PARAMETER_COUNT = 2 + len(POSITIONS) * (len(LANES) - 1)


def structured_log_pool_probabilities(
...
def fit_structured_log_pool_newton(
    ...
    *,
    ...
    lower_bounds[:2] = (0.0, 0.05)
    upper_bounds[:2] = (4.0, 4.0)
    converged = False
    iterations = 0
    ...
    return {
        ...
        "position_lane_coefficients": {
            f"position_{position + 1}_lane_{lane}": float(
                coefficients[_position_lane_index(position, lane)]
            )
...
def structured_probability_metrics(
    ...
    market_top5_hits = 0
    for race in races:
        probabilities = structured_probabilities(race, calibrator)
...
def structured_probabilities(
    ...
    return structured_log_pool_probabilities(
...
def select_structured_regularization_prequential(
    races: list[dict[str, Any]],
    *,
    ...
    for regularization in regularizations:
        ...
        for index in range(1, len(dates)):
            ...
            calibrator = fit_structured_log_pool_newton(
                training,
                regularization=float(regularization),
            )
            metrics = structured_probability_metrics(holdout, calibrator)
    ...
    final_calibrator = fit_structured_log_pool_newton(
        races,
        regularization=float(selected["regularization"]),
    )
    return {
        "validation_design": (
            "Regularization is selected on forward-only full-day folds; the "
            "17-parameter structure is fixed before the evaluation day"
        ),
        "dates": dates,
...
def _position_lane_index(position: int, lane: int) -> int:
    if position not in POSITIONS or lane not in LANES[1:]:
        raise ValueError("only lanes 2-6 have explicit position coefficients")
    return 2 + position * (len(LANES) - 1) + lane - 2
...
def _combination_features(
    ...
    *,
    ...
    for row_index, combination in enumerate(combinations):
        lanes = tuple(int(value) for value in combination.split("-"))
        if len(lanes) != 3 or len(set(lanes)) != 3 or any(lane not in LANES for lane in lanes):
            raise ValueError(f"invalid trifecta combination: {combination}")
        features[row_index, 0] = math.log(max(EPSILON, float(model[combination])))
        features[row_index, 1] = math.log(max(EPSILON, float(market[combination])))
        for position, lane in enumerate(lanes):
            if lane != 1:
                features[row_index, _position_lane_index(position, lane)] = 1.0
    return features
