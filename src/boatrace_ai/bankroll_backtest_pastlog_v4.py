...
from .features_pastlog_v4 import load_training_examples
from .modeling_pastlog_v4 import FEATURE_SET, make_pipeline, positive_probs
...
def bankroll_backtest_pastlog_v4(
    conn,
    *,
    output_path: Path,
    ...
    result["model"] = "win_model_pastlog_v4"
    result["policy"]["model"] = "win_model_pastlog_v4"
    result["policy"]["feature_set"] = FEATURE_SET
    result["policy"]["role"] = "primary_pastlog_v4"
    output_path.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Backtest daily bankroll buying with past-log v4 model.")
    parser.add_argument("--db", default="data/boatrace.sqlite")
    parser.add_argument("--output", default="data/models/bankroll_backtest_pastlog_v4_10000.json")
    ...
    with connection(args.db) as conn:
        result = bankroll_backtest_pastlog_v4(
            conn,
