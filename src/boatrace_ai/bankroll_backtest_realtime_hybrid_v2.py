...
from .features_realtime_hybrid_v2 import load_training_examples
from .modeling_realtime_hybrid_v2 import FEATURE_SET, make_pipeline, positive_probs
...
def bankroll_backtest_hybrid_v2(
    conn,
    *,
    output_path: Path,
    ...
    result["model"] = "win_model_realtime_hybrid_v2"
    result["policy"]["model"] = "win_model_realtime_hybrid_v2"
    result["policy"]["feature_set"] = FEATURE_SET
    result["policy"]["role"] = "shadow_realtime_hybrid_v2"
    result["policy"]["coverage_note"] = "historical realtime odds/beforeinfo coverage is sparse; treat bankroll result as a guardrail."
    output_path.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Backtest daily bankroll buying with realtime hybrid v2 shadow model.")
    parser.add_argument("--db", default="data/boatrace.sqlite")
    parser.add_argument("--output", default="data/models/bankroll_backtest_realtime_hybrid_v2_10000.json")
    ...
    with connection(args.db) as conn:
        result = bankroll_backtest_hybrid_v2(
            conn,
