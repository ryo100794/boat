...
from .features_realtime_hybrid_v2 import load_training_examples, prediction_features
from .modeling import _normalize_lane_probs, trifecta_predictions
from .modeling_no_odds_v6 import SparseIndex32


FEATURE_SET = "realtime_hybrid_v2_prevday_pastlog_beforeinfo_odds_logreg_C0.06_shadow"
...
def train_model(conn, *, model_path: Path, min_examples: int = 100) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "vectorizer": "sparse",
        "scaler": "MaxAbsScaler",
        "classifier": "LogisticRegression(liblinear, C=0.06, class_weight=None)",
        "feature_set": FEATURE_SET,
        "role": "shadow_realtime_hybrid",
        "design_note": "prior-date past-log rolling features plus latest beforeinfo/weather and odds aggregate features.",
    }
...
def backtest_model(
    ...
    *,
    ...
    result["shadow_note"] = "historical realtime beforeinfo/odds coverage is sparse; live shadow accuracy is required before promotion."
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train/backtest/predict realtime hybrid v2 shadow model.")
    sub = parser.add_subparsers(dest="command", required=True)
    train = sub.add_parser("train")
    add_common(train)
    train.add_argument("--model", default="data/models/win_model_realtime_hybrid_v2.joblib")
    train.add_argument("--min-examples", type=int, default=100)
    train.set_defaults(func=_cmd_train)
    backtest = sub.add_parser("backtest")
    add_common(backtest)
    backtest.add_argument("--output", default="data/models/backtest_realtime_hybrid_v2.json")
    ...
    add_common(predict)
    predict.add_argument("--model", default="data/models/win_model_realtime_hybrid_v2.joblib")
    predict.add_argument("--date", default=date.today().isoformat())
    predict.add_argument("--output", default="data/models/shadow_realtime_hybrid_v2_latest.json")
