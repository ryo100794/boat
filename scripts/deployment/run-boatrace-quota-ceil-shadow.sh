...
POLL_SECONDS="${BOATRACE_QUOTA_CEIL_SPEC_POLL_SECONDS:-10}"
BUNDLE="${BOATRACE_QUOTA_CEIL_BUNDLE:-$APP_ROOT/data/models/daily-shadow-bundles/2026-08-02/quota-ceil-job-9906.joblib}"
...
while true; do
  ...
  if [[ "$identity" != "$active_identity" ]]; then
    stop_child
    base_model="${v21_spec##*:}"
    export BOATRACE_T300_SHADOW_MODEL_SPEC="quota_ceil_daily:quota_ceil_v21_t300:$BUNDLE:$base_model"
    ...
    export BOATRACE_T300_SHADOW_MODULE="boatrace_ai.runtime.quota_ceil_shadow"
    "$SHADOW_RUNNER" &
    child_pid=$!
