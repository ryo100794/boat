...
if [[ -f "$OLD/postgresql/README.md" ]]; then
  mv "$OLD/postgresql/README.md" "$PG/README.md"
fi
...
rm -f "$OLD/boatrace-runtime/conf/postgresql-runtime.env"
rmdir "$OLD/postgresql/run" "$OLD/postgresql" 2>/dev/null || true
rmdir "$OLD/boatrace-runtime/conf" "$OLD/boatrace-runtime/data" "$OLD/boatrace-runtime" 2>/dev/null || true
