...
POLL_SECONDS="${BOATRACE_RAW_GUARD_SPEC_POLL_SECONDS:-10}"
BUNDLE="${BOATRACE_RAW_GUARD_BUNDLE:-$APP_ROOT/data/models/daily-shadow-bundles/2026-08-02/raw-guard-job-9906.joblib}"
...
while true; do
  ...
  if [[ "$identity" != "$active_identity" ]]; then
    stop_child
    base_model="${v21_spec##*:}"
    export BOATRACE_T300_SHADOW_MODEL_SPEC="raw_guard_daily:raw_guard_v21_t300:$BUNDLE:$base_model"
    ...
    export BOATRACE_T300_SHADOW_MODULE="boatrace_ai.runtime.raw_guard_shadow"
    "$SHADOW_RUNNER" &
    child_pid=$!
