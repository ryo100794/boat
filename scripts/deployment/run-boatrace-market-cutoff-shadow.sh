...
LOCK="${BOATRACE_MARKET_CUTOFF_SHADOW_LOCK:-$APP_ROOT/run/market-calibrated-cutoff-shadow.lock}"
...
exec 9>"$LOCK"
if ! flock -n 9; then
  echo "Cutoff-refit market shadow cycle is already running" >&2
  exit 75
fi
...
exec env \
  ...
  --db "$DSN" \
  --model "$APP_ROOT/data/models/listwise_newton_cutoff_20260717.joblib" \
  --output "$APP_ROOT/data/models/listwise_market_calibrated_cutoff_shadow.json" \
  --state "$APP_ROOT/data/models/listwise_market_calibrated_cutoff_shadow_state.json" \
