...
LOCK="${BOATRACE_MARKET_RESIDUAL_SHADOW_LOCK:-$APP_ROOT/run/market-residual-shadow.lock}"
...
exec 9>"$LOCK"
if ! flock -n 9; then
  echo "Newton market residual shadow cycle is already running" >&2
  exit 75
fi
...
exec env \
  ...
  --output "$APP_ROOT/data/models/listwise_market_residual_shadow.json" \
  --state "$APP_ROOT/data/models/listwise_market_residual_shadow_state.json" \
  --scored-cache "$APP_ROOT/data/models/listwise_market_calibrated_shadow.races.joblib" \
