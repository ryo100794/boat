...
LOCK="${BOATRACE_MARKET_BLEND_SHADOW_LOCK:-$APP_ROOT/run/market-calibrated-blend-shadow.lock}"
...
exec 9>"$LOCK"
if ! flock -n 9; then
  echo "Preselected blend market shadow cycle is already running" >&2
  exit 75
fi
...
exec env \
  ...
  --db "$DSN" \
  --model "$APP_ROOT/data/models/stagewise_blend_preselected_20260717.joblib" \
  --output "$APP_ROOT/data/models/stagewise_blend_market_shadow.json" \
  --state "$APP_ROOT/data/models/stagewise_blend_market_shadow_state.json" \
